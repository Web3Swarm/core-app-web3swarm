  ...
  public async setWebhook(webhookUrl: string): Promise<void> {
    try {
      if (
        ...
        process.env.TELEGRAM_BOT_TOKEN === "your_telegram_token"
      ) {
        console.warn(
          "[telegram.service] No valid Telegram bot token found. Skipping webhook setup."
        );
        return;
      }

      await this.bot.api.setWebhook(`${webhookUrl}/webhook/telegram`);
    } catch (error) {
      console.warn("[telegram.service] Failed to set webhook:", error.message);
    }
  }
  ...
  public async start(): Promise<void> {
    try {
      ...
      if (
        ...
        console.warn(
          "[telegram.service] No valid Telegram bot token found. Telegram functionality will be disabled."
        );
        return;
      }
      ...
      try {
        ...
        console.warn(
          "[telegram.service] Unable to use twitter. Functionality will be disabled",
          error
        );
      }
      ...
      try {
        ...
        console.warn(
          "[telegram.service] Failed to set bot commands:",
          error.message
        );
      }
      ...
      try {
        ...
        console.warn("[telegram.service] Failed to start bot:", error.message);
      }
    } catch (error) {
      console.error("[telegram.service] Failed to initialize:", error);
      throw error;
    }
  }

  private async registerCommandHandlers() {
    this.bot.command("start", (ctx) => ctx.reply("GOod Hello!  good"));
...
You can view the token page below (it takes a few minutes to be visible)`,
    ...
    this.bot.command("lit", async (ctx) => {
      try {
        ...
        switch (action) {
          ...
          case "decrypt-action": {
            ...
            const { data } = await axios.post(
              `${getCollablandApiUrl()}/telegrambot/executeLitActionUsingPKP?chainId=${chainId}`,
              {
        ...
        if (isAxiosError(error)) {
          console.error("Failed to execute Lit action:", error.response?.data);
          ctx.reply(
